...
class Picking(models.Model):
	...
	purchase_id = fields.Many2one('purchase.order', string='Purchase Orders', store=True)
	target_year = fields.Many2one('fin.year', string='Financial Year')
	# bill_fin_year = fields.Many2one('fin.year', 'Financial Bill Year', related='purchase_id.bill_fin_year')
	...
	def change_target_year(self):
		self.purchase_id.write({'bill_fin_year': self.target_year.id})

class StockPackOperation(models.Model):
	_inherit = "stock.pack.operation"

	partner_id = fields.Many2one(related="picking_id.partner_id", string="Partner", store=True)
	gstin_id = fields.Many2one('gstin.line', string="GSTIN", domain="[('line_id', '=', partner_id)]")
	# gstin_id = fields.Many2one('gstin.line', string="GSTIN", domain="[('line_id', '=', picking_id.partner_id.), ('supplier', '=', True)]")
	...
	art_work_id = fields.Many2one('art.work', string="Art Work")
	blister_image = fields.Binary(related="product_id.art_work_id.blister_image", string="Blister Image", store=True, readonly="1")
	duplex_image = fields.Binary(related="product_id.art_work_id.duplex_image", string="Duplex Image", store=True, readonly="1")
	lable_image = fields.Binary(related="product_id.art_work_id.label_image", string="Lable Image", store=True, readonly="1")

	@api.onchange('bill_no')
	def change_bill_no(self):
		b_ns = self.picking_id.purchase_id.bill_no
		if b_ns:
			b_n_l = b_ns.split(",")
			if self.bill_no:
				if self.bill_no in b_n_l:
					pass
				else:
					self.picking_id.purchase_id.write({'bill_no': b_ns+','+self.bill_no})
			else:
				self.picking_id.purchase_id._bill_calc()
		else:
			self.picking_id.purchase_id.write({'bill_no':self.bill_no})
