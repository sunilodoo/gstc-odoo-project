...
class saleOrderGST(models.Model):
	...
	validity_date = fields.Date(string='Expiration Date', readonly=False, copy=False, help="Manually set the expiration date of your quotation (offer), or it will set the date automatically based on the template if online quotation is installed.")
	...
	def custom_invoice(self):
		...
		for order in self:
			vals = {
				...
				'incoterms': order.incoterms and order.incoterms.id or '',
				'is_consignee': order.is_consignee and True or False,
				'consignee': order.consignee and order.consignee.id or '',
				'order_no': order.buyer_ref or order.name or '',
				'order_date': order.buyer_date or order.confirmation_date,
				...
				'ext_charge': order.ext_chrgs,
				'global_disc': order.global_disc,
				'total_ntwt': order.total_nwt_crtn_total,
				'total_grwt': order.total_gwt_crtn_total,
			}
			invoice_id = self.env['account.invoice'].create(vals)
			for line in order.order_line:
				self.env['account.invoice.line'].create({
					'product_id': line.product_id.id,
					'iinvoice_id': invoice_id.id,
					'name':line.name,
	...
	def _prepare_invoice(self):
		...
		res.update({
			...
			'order_no': self.buyer_ref and self.buyer_ref or self.name,
			'order_date': self.buyer_date and self.buyer_date or self.confirmation_date,
		    ...
		    for line in order.order_line.sorted(key=lambda l: l.qty_to_invoice < 0):
		        ...
		        if group_key not in invoices:
		            ...
		            if order.name not in invoices_origin[group_key]:
		                ...
		                ext_charge = ext_charge + order.ext_chrgs
...
class SaleOrderLine(models.Model):
	...
	def _prepare_invoice_line(self, qty):
		...
		print"-----------custom-----------",self,res
		res.update({
			'order_qty': self.product_uom_qty,
			'hsn_code': self.hsn_code and self.hsn_code.id,
		})
		return res	
