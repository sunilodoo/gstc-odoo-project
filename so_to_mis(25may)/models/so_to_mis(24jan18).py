...
class SaleOrder(models.Model):
	...
	def change_priority(self):
		...
		for mkt in summary_mkt:
			mkt.write({'so_priority': self.priority,})
		for po in summary_po:
			po.write({'so_priority': self.priority,})
...
class PurchaseOrder(models.Model):
	...
	def change_date(self):
		...
		if self.date_order and self.oe_dlvy_req_date :
			diff = datetime.datetime.strptime(self.pack_docs_day, '%Y-%m-%d') - datetime.datetime.strptime(self.date_order, '%Y-%m-%d %H:%M:%S')
			self.days_dispatch = diff.days
		for mkt in summary_mkt:
			mkt.write({'oe_dlvy_req_date': self.oe_dlvy_req_date, 'pack_docs_day': self.pack_docs_day, 'days_dispatch': self.days_dispatch})
		for po in summary_po:
			po.write({'oe_dlvy_req_date': self.oe_dlvy_req_date, 'pack_docs_day': self.pack_docs_day, 'days_dispatch': self.days_dispatch})
	...
	def change_valu(self):
		...
		summary_po = self.env['po_item.history2'].search([('po_id.name','=',self.name)])
		for mkt in summary_mkt:
			mkt.write({'dlvy_confm_date': self.dlvy_confm_date, 'dlvy_req_date': self.dlvy_req_date, 'cur_dlvy_date': self.cur_dlvy_date, 'act_dlvy_date': self.act_dlvy_date, 'inv_date': self.inv_date, 'bill_no': self.bill_no, 'bill_date': self.bill_date, 'sob_date': self.sob_date, 'doc_bank_date': self.doc_bank_date})
		for po in summary_po:
			po.write({'dlvy_confm_date': self.dlvy_confm_date, 'dlvy_req_date': self.dlvy_req_date, 'cur_dlvy_date': self.cur_dlvy_date, 'act_dlvy_date': self.act_dlvy_date, 'inv_date': self.inv_date, 'bill_no': self.bill_no, 'bill_date': self.bill_date, 'sob_date': self.sob_date, 'doc_bank_date': self.doc_bank_date})
		print"------------summary_mkt-----------summary_po----",summary_mkt,summary_po
	...
	@api.onchange('priority','proc_name')
	def change_priority(self):
		summary_mkt = self.env['po_item.history'].search([('po_id.name','=',self.name)])
		summary_po = self.env['po_item.history2'].search([('po_id.name','=',self.name)])
		print"------------change_priority------",summary_mkt,self.id,summary_po
		for mkt in summary_mkt:
			mkt.write({'priority': self.priority, 'proc_name':self.proc_name.id})
		for po in summary_po:
			po.write({'priority': self.priority, 'proc_name': self.proc_name.id})
