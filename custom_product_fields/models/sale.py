...
class SaleOrderLine(models.Model):
	...
	edd = fields.Char('EADD Days')
	edrg = fields.Date('EDRG')
	adrg = fields.Date('ADRG')
	vendor_id = fields.Many2one('res.partner', string='Vendor')
	bd_flag = fields.Boolean('2BD Flag')
	remarks = fields.Text('Problem')
	attach_ment = fields.Binary('Attachment')
	art_work_id = fields.Many2one('art.work', string="Art Work", domain="[('product_id', '=', product_id)]")
	blister_image = fields.Binary(related="art_work_id.blister_image", string="Blister Image", store=True, readonly="1")
	duplex_image = fields.Binary(related="art_work_id.duplex_image", string="Duplex Image", store=True, readonly="1")
	lable_image = fields.Binary(related="art_work_id.label_image", string="Lable Image", store=True, readonly="1")
	#qty_per_carton = fields.Integer('Qty/Ctn')
	#ctn_len = fields.Float("Length")
	#ctn_widt = fields.Float("Width")
	#ctn_hait = fields.Float("Height")
	...
	def onchange_vendor_id(self):
		print"-----qty------cus---------------"
		for rec in self:
			if rec.vendor_id and rec.vendor_id.name == '2BD':
				rec.bd_flag = True
			else:
				rec.bd_flag = False
