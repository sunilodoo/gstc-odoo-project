...
class SaleOrder(models.Model):
	...
	shipping_import = fields.One2many('shipping.import', 'order_id', string="Shipping Import")
	shipping_export = fields.One2many('shipping.export', 'order_id', string="Shipping Export")
	target_year = fields.Many2one('fin.year', string='Financial Year', track_visibility='always')
	...
	def onchange_country(self):
		...
		#	order.partner_country_id = order.partner_id and order.partner_id.country_id.id or False
		self.partner_country_id = self.partner_id and self.partner_id.country_id.id or False
			...
			for po in po_id:
				for p_line in po.order_line:
					...
					if (line.product_id == p_line.product_id) and po.problem:
						remarks = po.problem
					if (line.product_id == p_line.product_id) and (po.act_dlvy_date and po.dlvy_confm_date):
						print"====================111111111111111"
						
						date1 = datetime.datetime.strptime(po.dlvy_confm_date, '%Y-%m-%d') 
						li.append(date1)
						date2 = datetime.datetime.strptime(po.act_dlvy_date, '%Y-%m-%d')
						li.append(date2)
						m = max(li)
						edd_diff = m - datetime.datetime.strptime(self.confirmation_date, '%Y-%m-%d')
						edd = edd_diff.days
	...
	def _get_invoiced(self):
		...
		for order in self:
			summary_mkt = self.env['po_item.history'].search([('so_id.name','=',order.name)])
			summary_po = self.env['po_item.history2'].search([('so_id.name','=',order.name)])
	...
	@api.onchange('date_order','confirmation_date')
	def change_year(self):
		print"-----------self---change_year-------",self.date_order,self.confirmation_date
		year = ''
		month = ''
		if self.date_order and not self.confirmation_date:
			year = datetime.datetime.strptime(self.date_order, '%Y-%m-%d %H:%M:%S').strftime('%Y')
			month = datetime.datetime.strptime(self.date_order, '%Y-%m-%d %H:%M:%S').month
			if month <= 3:
				target_year = self.env['fin.year'].search([('name','like', '-'+str(year[2:]))])
			if month > 3:
				target_year = self.env['fin.year'].search([('name','ilike', year)])
				for tg in target_year :
					self.target_year = tg.id

			# self.target_year = target_year.id
		if self.date_order and self.confirmation_date:
			year = datetime.datetime.strptime(self.confirmation_date, '%Y-%m-%d').strftime('%Y')
			month = datetime.datetime.strptime(self.confirmation_date, '%Y-%m-%d').month
			if month <= 3:
				target_year = self.env['fin.year'].search([('name','like', '-'+str(year[2:]))])
			if month > 3:
...
class ShippingImport(models.Model):
	_name = "shipping.import"
	_order = "edrg desc"
	...
	port_of_delivery = fields.Char('Port of Delivery')
	shipping_method = fields.Many2one('shipping.method', string='Shipping Method')
	shipping_agent = fields.Selection([('buyer','Buyer'),('self','Self')], string='Shipping Agent')
	specific_sl = fields.Many2one('specific.sl', string="Specific SL")
	remarks = fields.Text('Remarks')

class ShippingExport(models.Model):
	_name = "shipping.export"
	_order = "edrg desc"
...
class PurchaseOrder(models.Model):
	...
	@api.depends('order_line.price_total','freight','tcs_rate')
	def _amount_all(self):
		for order in self:
			...
			for line in order.order_line:
				...
				tcs_amount = order.currency_id.round((amount_untaxed + amount_tax + order.freight)*order.tcs_rate*0.01)
			order.update({
				'amount_untaxed': order.currency_id.round(amount_untaxed),
				'amount_tax': order.currency_id.round(amount_tax),
				'tcs_amount': tcs_amount,
				'amount_total': amount_untaxed + amount_tax + order.freight+order.tcs_amount,
			})
	...
	dlvy_confm_date = fields.Date('DLVY Confirm date', copy=False)
	dlvy_req_date = fields.Date('DLVY req. Date', copy=False)
	oe_dlvy_req_date = fields.Date('Original EXP DLVY date', copy=False)
	cur_dlvy_date = fields.Date('Current DLVY date', copy=False)
	act_dlvy_date = fields.Date('Actual DLVY date', copy=False)
	inv_date = fields.Date('Invoice date', copy=False)
	# bill_no = fields.Char('Bill No', copy=False)
	# bill_no = fields.Char('Bill No', compute='_bill_calc', store=True)
	bill_no = fields.Char('Bill No', copy=False, readonly=False)
	...
	problem = fields.Text('Problems', track_visibility='always')
	...
	edd = fields.Date("EDD")
	import_mode = fields.Selection([('hss','HSS'),('india','India')], string='Import Mode')
	target_year = fields.Many2one('fin.year', string='Financial Year', track_visibility='always')
	bill_fin_year = fields.Many2one('fin.year', string='Billing Financial Year')
	freight = fields.Float(string="Freight")
	tcs_rate = fields.Float(string="TCS@0.10%")
	tcs_amount = fields.Float(string="TCS Amount")
	date_planneds = fields.Datetime(string='Scheduled Date')
	@api.onchange('tcs_rate')
	def change_tcs_rate(self):
		self._amount_all

	# @api.one
	# @api.depends('picking_ids.target_year')
	# def _get_bill_fin_year(self):
	# 	print"-------------state-------state-----------"
	# 	for line in self:
	# 		if line.picking_ids:
	# 			line.bill_fin_year = line.picking_ids[0].target_year.id
	# @api.depends('picking_ids')
	# @api.multi
	# @api.depends('picking_ids.pack_operation_product_ids.bill_no')
	# # @api.depends('picking_ids')
	def _bill_calc(self):
		print"-----------bill_calc---------",self
		bill_no = ''
		print"-----------self.picking_ids---------",self.picking_ids
		for p_l in self.picking_ids:
			print"--1----p_l", p_l
			print"------qwe-----p_l---------",p_l
			for b_l in p_l.pack_operation_product_ids:
				print"--2----b_l", b_l
				# print"--2----b_l.bill_no----", b_l.bill_no
				if b_l.bill_no:
					bill_no = bill_no+','+b_l.bill_no
					# print"-------bl--------", bill_no
		self.write({'bill_no': bill_no})
	...
	def _get_invoiced(self):
		...
		for order in self:
			summary_mkt = self.env['po_item.history'].search([('po_id.name','=',order.name)])
			summary_po = self.env['po_item.history2'].search([('po_id.name','=',order.name)])
			print"------------summary_mkt---------------",summary_mkt,order.partner_id,order.state
			for mkt in summary_mkt:
				mkt.write({'state':order.state,})
			for po in summary_po:
				po.write({'state':order.state})
	...
	def _prepare_picking(self):
		...
		if self and self.origin and 'SO' in self.origin:
			so = self.env['sale.order'].search([('name','ilike',self.origin[:5])], limit=1)
			so_id = so.id
		else:
			so_id = False
		if self.hss == True:
	...
	def change_order_date(self):
		...
		if self and self.origin and 'SO' in self.origin:
			...
			if so and so.confirmation_date :
	...
	@api.onchange('date_order')
	def change_year(self):
		print"-----------self---change_year-------",self.date_order
		year = ''
		month = ''
		if self.date_order:
			year = datetime.datetime.strptime(self.date_order, '%Y-%m-%d %H:%M:%S').strftime('%Y')
