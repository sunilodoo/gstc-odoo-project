...
class POMisReporting(models.Model):
	...
	delay_reason = fields.Many2one('delay.reason', string='ROD', track_visibility='always')
	...
	balance = fields.Float(compute='calc_balance', string='Balance amount')
	...
	target_year = fields.Many2one('fin.year', string='Financial Year', default=6)
	...
	color = fields.Selection([('deeppink','Pink'),('black','Black'),('green','Green'),('grey','Grey'),('magenta','Magenta'),('brown','Brown'),('cyan','Cyan')], string='Color')
	...
	@api.depends('amount','recvd_amount')
	def calc_balance(self):
		#print"--------------======-------",self.edrg
		balance = 0
		for rec in self:
			balance = rec.amount - rec.recvd_amount
			rec.balance = balance
...
class PaymentStatus(models.Model):
	...
	def onchange_color(self):
		print"----------self.color-----",self.color,self.id
		po_mis = self.env['po_mis.reporting'].search([('payment_status.name','=',self.name)])
		if po_mis:
			for mis in po_mis:
				mis.write({'color': self.color})
		print"-----po_mis----",po_mis
